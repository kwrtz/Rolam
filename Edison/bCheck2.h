...
public:
    ...
    virtual void onInitialize(Blackboard& bb) {
        if( state == 0) {
            ...
            if(  time-bb.perimeterSensoren.lastTimeSignalReceivedL > 30000) {
                state = 1;
    ...
    virtual NodeStatus onUpdate(Blackboard& bb) {
        ...
        if(state != 0) {
            unsigned long time = millis();
            if( time-bb.perimeterSensoren.lastTimeSignalReceivedL < 800 ) {
                state = 0;
...
public:
    ...
    virtual void onInitialize(Blackboard& bb) {
        if( state == 0) {
            unsigned long time = millis();
            if(  time-bb.perimeterSensoren.lastTimeSignalReceivedR > 30000) {
                state = 1;
    ...
    virtual NodeStatus onUpdate(Blackboard& bb) {
        ...
        if(state != 0) {
            unsigned long time = millis();
            if( time-bb.perimeterSensoren.lastTimeSignalReceivedR < 800 ) {
                state = 0;
...
class TCheck2BackCoilSignal: public Node
{
private:
    int state;
public:

    TCheck2BackCoilSignal() {
        state = 0;
    }

    virtual void onInitialize(Blackboard& bb) {
        if( state == 0 && bb.flagGotoAreaXFirstCall == false) {
            unsigned long time = millis();
            if(  time-bb.perimeterSensoren.lastTimeSignalReceivedB > 30000 ) {
                state = 1;
                sprintf(errorHandler.msg,"!03,Signal Back Lost\r\n");
                errorHandler.setInfo();
            }
        }
    }
    ...
    virtual NodeStatus onUpdate(Blackboard& bb) {
        ...
        if(state != 0) {
            unsigned long time = millis();
            if( time-bb.perimeterSensoren.lastTimeSignalReceivedB < 800 ) {
                state = 0;
                sprintf(errorHandler.msg,"!03,Signal Back Found\r\n");
                errorHandler.setInfo();
            }
...
class TCheck2BackCoilSignalAreaX: public Node
{
private:
    int state;
    ...
    virtual NodeStatus onUpdate(Blackboard& bb) {
        ...
        if(state != 0) {
            ...
            if( state == 1) {
                ...
                if( bb.perimeterSensoren.isBackInside() ||  bb.perimeterSensoren.isBackOutside() ) {
                    state = 0;
                    bb.flagGotoAreaXFirstCall = false;
                    sprintf(errorHandler.msg,"!03,Signal Back Found SignalAreaX1\r\n");
                    errorHandler.setInfo();
                    return BH_FAILURE;
                }
            ...
            if( state == 2) {
                if ( bb.motor.isPositionReached()) {
                    state = 0;
                    bb.flagGotoAreaXFirstCall = false;
                    sprintf(errorHandler.msg,"!03,TCheck2BackCoilSignalAreaX rotated 10deg\r\n");
                    errorHandler.setInfo();
                    return BH_FAILURE;
                }
                return BH_RUNNING;
...
public:
    ...
    virtual void onInitialize(Blackboard& bb) {
        if( state == 0) {
            unsigned long time = millis();
            if(  (time-bb.perimeterSensoren.lastTimeSignalReceivedL > 1000) &&
                    (time-bb.perimeterSensoren.lastTimeSignalReceivedR > 1000) &&
    ...
    virtual NodeStatus onUpdate(Blackboard& bb) {
        ...
        if(state != 0) {
            ...
            if(  (time-bb.perimeterSensoren.lastTimeSignalReceivedL < 800) &&
                    (time-bb.perimeterSensoren.lastTimeSignalReceivedR < 800) &&
        ...
        if( state == 0) {
            if( bb.perimeterSensoren.isLeftOutside() && bb.perimeterSensoren.isRightOutside() && bb.perimeterSensoren.isBackOutside()) {
                if( bb.perimeterSensoren.magnetudeL < 0 && bb.perimeterSensoren.magnetudeR < 0  && bb.perimeterSensoren.magnetudeB < 0 ) { // Es kann sein dass die Zähler etwas träge sind.
                    ...
                    errorHandler.setInfo();
                    sprintf(errorHandler.msg,"!03,ML: %d MR: %d MB: %d CL:%d CR: %d CB: %d\r\n",bb.perimeterSensoren.magnetudeL , bb.perimeterSensoren.magnetudeR, bb.perimeterSensoren.magnetudeR , bb.perimeterSensoren.signalCounterL ,bb.perimeterSensoren.signalCounterR, bb.perimeterSensoren.signalCounterB );
                    errorHandler.setInfo();
                }
            }
        }
