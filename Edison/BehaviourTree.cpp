...
NodeStatus BehaviourTree::tick(Blackboard& bb)
{
    ...
    // Show current node only if node changes from last run and flagShowLastNode is set
    if(flagShowLastNode && bb.lastNodeCurrentRun!=NULL) {
...
#ifdef DEBUG_BHT_STACK
    debug->printf( "######Stack lastRunningNodes\r\n");
    int end1;
    end1 = lastRunningNodes.size;
    debug->printf( "lastRunningNodes StackSize: %d\r\n", end1);
    for (int i=0; i<end1; i++) {
        Node* n = lastRunningNodes[i];
        debug->printf( "Node: %s %d\r\n", n->nodeName.c_str(), n->nodeId);
    }

    debug->printf( "######Stack runningNodes\r\n");
    end1 = bb.runningNodes.size;
    debug->printf( "runningNodes StackSize: %d\r\n", end1);
    for (int i=0; i<end1; i++) {
        Node* n = bb.runningNodes[i];
        debug->printf( "Node: %s %d\r\n",n->nodeName.c_str(), n->nodeId);
    }
    debug->printf( "######STACK END\r\n");
#endif
    ...
    for (int i=0; i< end; i++) {
        ...
        if (last->nodeId != current->nodeId ) {
            start = i;
            DSTACK(debug->printf( "lastNode % s %d != currentNode %s %d\r\n", last->nodeName.c_str(), last->nodeId, current->nodeName.c_str(), current->nodeId);)
            break;
        }
    }
...
#ifdef DEBUG_BHT_STACK
    debug->printf( "######Stack lastRunningNodes populated\r\n");
    end1 = lastRunningNodes.size;
    debug->printf( "lastRunningNodes StackSize: %d\r\n", end1);
    for (int i=0; i<end1; i++) {
        Node* n = lastRunningNodes[i];
