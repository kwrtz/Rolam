...
void I2C_reset() {
	while (true) {
		...
		if (rtn == 0) return;
		DEBUGLN(F("I2C bus error. Could not clear"));
		if (rtn == 1) {
			DEBUGLN(F("SCL clock line held low"));
		}
		else if (rtn == 2) {
			DEBUGLN(F("SCL clock line held low by slave clock stretch"));
		}
		else if (rtn == 3) {
			DEBUGLN(F("SDA data line held low"));
		}
	}
}
