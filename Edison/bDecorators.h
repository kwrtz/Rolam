...
class TdnMowModeWorkx: public DecoratorNode
{
public:

    virtual NodeStatus onUpdate(Blackboard& bb) {

        if ( bb.flagMowMode == MOWMODE_WORKX ) {
            NodeStatus s = m_pChild->tick(bb);
            return s;
        }
...
class TdnMowMode308: public DecoratorNode
{
public:

    virtual NodeStatus onUpdate(Blackboard& bb) {

        if ( bb.flagMowMode == MOWMODE_308 ) {
            NodeStatus s = m_pChild->tick(bb);
            return s;
        }
...
/*
class TdnPerOutsideActivated: public DecoratorNode
{
public:

    virtual NodeStatus onUpdate(Blackboard& bb) {

        if (bb.flagPerimeterActivated) {
            NodeStatus s = m_pChild->tick(bb);
            return s;
        }
    ...
    virtual void onTerminate(NodeStatus status, Blackboard& bb) {
        //if (status != BH_ABORTED) {
            ...
            DDNT(debug->printf("bb.flagPerimeterActivated  = false;\r\n");)
        //}
    }
};
*/
 ...
 /*     
        if(status == BH_ABORTED) {
            if(getTimeInNode()<6000) {
                bb.shortWayCounter++;
                if(bb.shortWayCounter>bb.SHORTWAYCOUNT) {
                    bb.shortWayCounter=bb.SHORTWAYCOUNT;
                }
            }
